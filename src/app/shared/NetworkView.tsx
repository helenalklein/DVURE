...
// Real "Invite Partner" flow — replaces the old fake Network tab (a
// hardcoded agency list whose "Add" button only touched local React
// state, never persisted anywhere) and gives Agency an equivalent
// screen for the first time. Closed-loop by design, per direct
// instruction: the pilot pairs brands/agencies that already know each
// other — this is not a directory for finding new partners, and won't
// become one until a real Phase 2 decision to open it up.
...
export default function NetworkView({ onMenuClick }: { onMenuClick?: () => void }) {
  const { org } = useAuth();
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 min-w-0">
      <TopBar title="Network" sub={`${otherSide === "Agencies" ? "Agency" : "Brand"} relationships and partners`} onMenuClick={onMenuClick}/>
        ...
        <div className="glass-subtle border rounded-md p-4 mb-6 space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div>
              ...
              <TextInput placeholder={otherSide === "Agencies" ? "e.g. Vantage Model Mgmt." : "e.g. Vellani"} value={orgName} onChange={e=>setOrgName(e.target.value)}/>
            </div>
          </div>
