...
import { MapPin, Car, Cross, CloudSun, Clock, Plus, X, Phone, Mail, Star } from "lucide-react";
...
// shifts it back a day for anyone west of UTC — confirmed live (a real
// Aug 18 shoot day displayed as "Mon, Aug 17"). Same bug class already
// fixed once elsewhere in this app for campaign due dates. Parsing the
// Y-M-D parts directly into a local-midnight Date sidesteps the UTC
// conversion entirely instead of fighting it.
...
export default function ShootCallSheet({ campaignId, campaignName }: { campaignId: string; campaignName: string }) {
  ...
  if (shootDays.length === 0) return <div className="p-6 text-sm text-muted-foreground">No shoot days on this campaign yet — add one to build a call sheet.</div>;
  ...
  return (
    <div className="h-full overflow-auto p-6 space-y-5">
      ...
      {editing ? (
        ...
        <>
          <div className="glass-subtle border rounded-md p-4 grid grid-cols-3 gap-4">
