...
export async function fetchOrgMembership(profileId: string) {
  return supabase
    ...
    .select("access_level, title, organizations(id, name, org_type, verification_status, subscription_status, trial_ends_at, logo_url, payment_locked, self_described_services)")
