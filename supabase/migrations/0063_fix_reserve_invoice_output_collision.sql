-- 0062's reserve_invoice_for_card_payment named its output column
-- invoice_id — colliding with the real invoice_id column referenced
-- unqualified elsewhere in the function body (the invoice_line_items
-- insert's column list), since PL/pgSQL treats OUT/table-return
-- columns as in-scope variables for the whole function. Renamed to
-- out_invoice_id, which nothing else in the function shares a name
-- with.
drop function reserve_invoice_for_card_payment(uuid, numeric, uuid, uuid, uuid);
...
returns table(out_invoice_id uuid, remaining_amount numeric)
