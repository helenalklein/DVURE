-- Real contract text, provided by the user (drafted with ChatGPT,
-- reformatted here into structured HTML matching how the RichTextEditor
-- renders a document -- h1/h2/h3 hierarchy, real lists, merge tags for
-- everything this platform actually tracks). Replaces the placeholder
-- text from 0092.
--
-- Merge tag choices, so a future reader knows why a given field is a
-- {{tag}} versus static text versus left as a blank fill-in:
--  - {{model_name}}, {{day_rate}}, {{territory}}, {{duration}},
--    {{brand_name}}, {{contract_number}} -- pre-existing tags, real
--    values the app already tracks per deal.
--  - {{project_name}}, {{model_email}} -- new tags added alongside this
--    migration (resolveContractMergeTags, contracts.ts) -- campaign
--    name and the model''s own email, both already
--    available where the contract is generated.
--  - Rate Basis ("Day Rate") and payment processor ("DVURE Payment
--    Processing") are hardcoded static text, not blanks -- both are
--    always true for how this platform actually operates, so leaving
--    them as fill-in brackets would be inaccurate, not just incomplete.
--  - Everything else genuinely deal-specific but not tracked anywhere
--    in the schema (addresses, call times, overtime rate, exclusivity
--    terms, etc.) is left as an unrecognized {{tag}} -- resolveContractMergeTags
--    already renders any tag outside its known set as a visibly blank,
--    labeled field rather than silently deleting it or leaking raw
--    {{}} syntax into a legal document.
--  - The 4.1 Permitted Media / 10. Travel checklists became real bullet
--    lists with every option present as plain editable text (not
--    blank-styled) -- a brand deletes the lines that don''t apply, which
--    is far more usable than a dozen individual blank-field chips.
--  - The DVURE RECORD block''s Status line is left blank rather than
--    hardcoded ''FULLY EXECUTED'' -- that would be actively false for
--    every contract at the moment it''s generated, before it''s even sent.
--
-- Scoped to rows that are STILL the placeholder (org_id is null -- the
-- global DVURE default -- or any brand''s own copy of it that hasn''t been
-- customized yet) so this can never overwrite a brand''s genuine edits.
...
<h2>1. Project</h2>
<p>Project Name: {{project_name}}<br>Project Type: {{project_type}}<br>Client / Brand: {{brand_name}}<br>Services / Role: {{services_description}}<br>Shoot / Performance Date(s): {{shoot_dates}}<br>Call Time: {{call_time}}<br>Estimated End Time: {{end_time}}<br>Location: {{shoot_location}}<br>Photographer / Production Company: {{production_company}}<br>Other Relevant Personnel: {{other_personnel}}</p>
...
<p>Legal Name: {{brand_name}}<br><br>By: _________________________________________<br><br>Name: _______________________________________<br><br>Title: ________________________________________<br><br>Date: ________________________________________</p>
...
where source = 'dvure_default'
  and content_html like '%Placeholder — DVURE Standard Contract%';
