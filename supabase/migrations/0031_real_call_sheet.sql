...
-- grid (campaign_crew_slots, now labeled "Crew" in the UI, which this
-- reuses the my_call_sheet_role() helper from but doesn't touch). One
-- call sheet per shoot day (shoot_days already models a campaign having
-- several), since that's how call sheets work in practice — a 3-day
...
-- up need to read it. Per direct decision, editing is simpler than
-- Crew's own tiering: admin/producer only, no per-department-lead
-- partial edit.
...
-- that don't fit a strict time type, same rationale this codebase
-- already applied to shoot_days.hours.
...
-- through the two RPCs below, matching this schema's established pattern
-- (0025/0026's own Crew tables work the same way).
...
-- shoot_days itself has never been seeded by any migration — the six
-- rows on the live database were created through real app usage, not a
-- migration, so a fresh database (a new dev environment, or this
-- worktree's own local `supabase db reset`) has zero shoot days and
-- nowhere to attach a call sheet. Guarded per-campaign so this is a
-- no-op against the live database (every one of its 3 real campaigns
-- already has shoot days) but gives a fresh install two real ones per
-- campaign to build call sheets against.
do $$
declare
  v_campaign record;
begin
  for v_campaign in select id, due_date from campaigns where brand_org_id = (select id from organizations where name = 'Vellani')
  loop
    if not exists (select 1 from shoot_days where campaign_id = v_campaign.id) then
      insert into shoot_days (campaign_id, date_label, event_date, sort_order) values
        (v_campaign.id, 'Day 1', coalesce(v_campaign.due_date, current_date), 1),
        (v_campaign.id, 'Day 2', coalesce(v_campaign.due_date, current_date) + 1, 2);
    end if;
  end loop;
end $$;
